...
void main() {
  group('SearchState', () {
    test('has correct initial status', () {
      expect(const SearchState.initial().status, equals(SearchStatus.initial));
    });

    group('copyWith', () {
      test('returns same object '
          'when no properties are passed', () {
        expect(SearchState.initial().copyWith(), equals(SearchState.initial()));
      });

      test('returns object with updated status '
          'when status is passed', () {
        expect(
          SearchState.initial().copyWith(status: SearchStatus.loading),
          equals(
            SearchState(
      ...
      test('returns object with updated articles '
      ...
      test('returns object with updated topics '
      ...
      test('returns object with updated searchType '
          'when searchType is passed', () {
        expect(
          SearchState.initial().copyWith(searchType: SearchType.relevant),
          equals(
            SearchState(
              topics: const [],
              status: SearchStatus.initial,
